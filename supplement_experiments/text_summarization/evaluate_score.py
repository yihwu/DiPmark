def pipeline(path = "/fs/nexus-scratch/ywu42/data_supp"):
    ...
    store_worker = Process(
        ...
        args=(f"{path}/text_summarization_score_old.txt", r2q, r2qe),
    )
...
def pipeline2():
    ...
    store_worker = Process(
        target=simple_store_worker,
        args=("data/text_summarization_ablation.txt", r2q, r2qe),
    )
    rt_worker.start()
    store_worker.start()
    ...
    for wp in wps:
        ...
        for model_str in ["facebook/bart-large-cnn"]:
...
#     store_worker = Process(
#         target=simple_store_worker,
#         args=(f"{path}/text_summarization_score.txt", r2q, r2qe),
#     )
...
def pipeline_dip(path = "/fs/nexus-scratch/ywu42/data_supp"):
    from supplement_experiments.common import set_spawn
    ...
    score_worker_ = [
        Process(
            target=dip_score_worker,
            args=(tq, tqe, rq, i, "daryl149/llama-2-7b-chat-hf")
